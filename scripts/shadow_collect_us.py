#!/usr/bin/env python3
"""
US Indo-Pacific (DVIDS) shadow collection run.

Reads `shadow/us_indopacom/manifest.json`, collects into a shadow SQLite
...
Isolation is the point. This script:
  ...
  * writes only inside `--state-dir`, which the workflow checks out from the
    `shadow/us-indopacom` branch and pushes back
...
from scraper.sources.us_dvids import (                         # noqa: E402
    REJECTION_REASONS, USDvidsAdapter)

MANIFEST = REPO_ROOT / "shadow" / "us_indopacom" / "manifest.json"
...
CREATE INDEX IF NOT EXISTS idx_us_shadow_published
    ON shadow_records(published_date);
"""
# `source_identity` is UNIQUE as well as `url` being the primary key. Two
# different URLs claiming the same DVIDS document id is a collision the
# database refuses rather than a duplicate the corpus absorbs quietly.
...
def assert_isolated(state_dir: Path) -> None:
    ...
    if REPO_ROOT in state_dir.parents or state_dir == REPO_ROOT:
        raise SystemExit(
            ...
            "tree: %s\nShadow state belongs on the shadow/us-indopacom "
...
def run(state_dir: Path, target: date, lookback: int, cap: int,
    ...
    entry = {
        ...
        # Every item the feed offered and this run did not keep, by reason.
    ...
    src = load_source()
    adapter = adapter or USDvidsAdapter(src, cap=cap)
    ...
    entry["rejections"] = dict(getattr(adapter, "rejections", entry["rejections"]))
    ...
    for ref in discovery.references:
        ...
        for doc in result.documents:
            ...
            row = conn.execute(
                ...
                "   OR source_identity = ?", (doc.url, identity)).fetchone()
            if row:
                entry["duplicates"] += 1
                continue
            conn.execute(
    ...
    elif entry["redirect_refusals"] and not entry["inserted"] \
...
def main(argv=None) -> int:
    ...
    if summary:
        with open(summary, "a", encoding="utf-8") as fh:
            fh.write("## US Indo-Pacific shadow run (DVIDS)\n\n")
            fh.write("Unit-tagged DVIDS public affairs, not a command-release "
                     "wire. See shadow/us_indopacom/README.md.\n\n")
            fh.write("| field | value |\n|---|---|\n")
            for k in ("result", "health", "shadow_day", "discovered",
                      "selected", "retrieved", "inserted", "duplicates",
                      "rejected_total", "fetch_failures",
                      "extraction_failures", "access_failures",
                      "redirect_refusals", "stored_total", "robots_status"):
                fh.write("| %s | %s |\n" % (k, entry.get(k)))
            fh.write("\n| rejected | count |\n|---|---|\n")
            for k, v in sorted(entry["rejections"].items()):
                if v:
                    fh.write("| %s | %s |\n" % (k, v))
