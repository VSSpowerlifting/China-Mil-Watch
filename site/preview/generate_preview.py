...
def edition_cover(repo_root: Path, slug: str, sidecar: dict):
    """
    The published cover for one edition, with the credit it was published with.

    Rights basis, which is the only reason this function is careful: the cover
    PNG is generated by this repository from HTML and CSS
    (`scripts/generate_pla_watch_cover.py` — deterministic, no AI images), and
    it has been public on the edition page since that edition shipped. It is a
    project-owned asset carrying its own recorded credit, which is why it may
    be reused here and why the source-article photograph it was composed over
    may not be used on its own: that photograph's reuse basis is not
    documented anywhere, and an undocumented basis is a no.

    Returns None when no cover exists. A missing cover renders nothing at all —
    never an empty frame, which would advertise an absence as a design element.
    """
    covers = repo_root / "output" / "the-pla-watch" / "covers"
    full = covers / (slug + ".png")
    if not full.is_file():
        return None
    credit = ""
...
def build(out_dir: Path, title: str, db_path: Path,
    ...
    ctx = {
        ...
        # registry. Not "sources configured": a configured source that is
        # switched off collects nothing, and counting it on the front page
        # would overstate the reach of the record by exactly the sources that
        # are not working.
    ...
    # into `the-pla-watch/`, for two reasons. The renderer is forbidden from
    # writing into the predecessor namespace at all, so it cannot put anything
    # there; and a candidate built into an empty directory has no carried
    # tree behind it, so a cross-reference would render as a broken image
    # everywhere except production. A build that only looks right in one
    # environment is a build nobody can review.
    #
    # Each is a project-generated PNG with a recorded digest — see
    # `edition_cover()` for the rights basis. `covers.json` is the manifest:
    # what was copied, from where, at what size, and with which credit.
    covers = [e["cover"] for e in editions if e.get("cover")]
    if covers:
        (out_dir / "cover").mkdir(parents=True, exist_ok=True)
        for cover in covers:
            (out_dir / cover["route"]).write_bytes(cover["source"].read_bytes())
        (out_dir / "covers.json").write_text(json.dumps({
            "note": ("Editorial covers reused on this site. Each is generated "
                     "by scripts/generate_pla_watch_cover.py from HTML and CSS "
                     "and was published with the edition it belongs to."),
            "covers": [{
                "route": "/" + c["route"],
                "edition": c["slug"],
                "sha256": c["sha256"],
